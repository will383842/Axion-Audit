...
| `pnpm check:no-skipped-tests`  | `ci.yml` job `anti-skip`                                    | Garde-fou anti-skip, **liste d'exceptions vide** (`scripts/check-no-skipped-tests.mjs`) ; périmètre `*.test.ts(x)` / `*.spec.ts(x)` / `*.e2e.ts` / `*.perf.ts` (bancs A28 ajoutés le 2026-09-09) |
