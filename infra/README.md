...
> **⚠️ CETTE CINQUIÈME TÂCHE N'EXISTE QUE SUR CE CHEMIN, QUI EST CELUI QUI N'A JAMAIS TOURNÉ.**
> La pile Coolify n'a pas d'hôte, donc pas de `cron` : `sonde-alertes.sh` n'y est planifiée par rien.
> Le dire ici est le minimum ; l'y porter est une DÉCISION D'INFRASTRUCTURE qui n'a pas été prise —
> le script s'appuie aujourd'hui sur `docker` (`axion_compose exec` pour PostgreSQL, un conteneur
> jetable pour lire le magasin TLS), et un side-car de la pile n'a pas le socket Docker (le lui
> donner serait une élévation de privilège, refusée pour le service `sauvegarde` par le même
> raisonnement). Le portage demande donc de remplacer ces deux accès par une connexion réseau à
> `postgres` et un montage en lecture seule de `caddy_data` — c'est une pièce distincte, chiffrée,
> à arbitrer. **Tant qu'elle n'est pas faite, l'invariant 8 est tenu sur le chemin décrit et PAS sur
> le chemin exploité.** Reporté au tableau §8, ligne 2i.
...
| 2i  | `sonde-alertes.sh` (fiche O-2) n'est planifiée que par `cron` : la pile Coolify — le chemin ÉPROUVÉ — ne l'exécute pas | **OUVERT — arbitrage à demander** ; encadré du §7.4. Le portage exige de remplacer `docker exec` par une connexion réseau et un montage lecture seule |
