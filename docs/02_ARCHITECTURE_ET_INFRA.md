...
*(27/08/2026. Complète §11. Ces décisions sont exécutables telles quelles par l'autopilote.)*
...
**═══ AMENDEMENT (2026-09-02, `DECISIONS.md` « Le statut du §30.6 est daté », décidé par Williams) ═══** La phrase ci-dessus était vraie **le 27/08/2026, à la rédaction**. Elle ne l'est plus telle quelle : entre le 27/08 et le 31/08, l'exécution du lot L0 a pris des décisions d'infrastructure que le pack ne prévoyait pas, toutes tracées dans `DECISIONS.md` et **ratifiées par Williams le 2026-08-31** — Coolify comme ordonnanceur (§30.1), Cloudflare R2 comme copie hors serveur (§11.4), CPX32 (§11.1), dépôt public pseudonymisé (§30.5), construction des images sur le serveur (§30.6 ci-dessus : le `pull` GHCR est remplacé par un `build` local), jeton de déploiement restreint, coffre de secrets chiffré. **La liste qui fait foi est `docs/conception/ALIGNEMENT_PACK_CODE.md` §2.** La porte P-A (2026-08-30) a acté que le lot L0 est exécutable de bout en bout **avec ces décisions**, pas sans question.
