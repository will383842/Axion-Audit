...
Node 22 LTS · pnpm 9 · TypeScript 5 (`strict: true`) · Fastify 5 (+ @fastify/jwt, @fastify/rate-limit, @fastify/helmet, @fastify/multipart, **@fastify/cookie** — AMENDEMENT du 2026-08-31, décidé par Williams, tracé dans `DECISIONS.md` : le §3 de ce contrat impose « cookies httpOnly SameSite=Lax + en-tête anti-CSRF » pour la console, et cette liste ne contenait aucun greffon capable de les poser. Livrer les routes admin en Bearer aurait signifié **écrire l'authentification de la console deux fois** et laisser une incohérence entre `apps/hq` et ce contrat) · PostgreSQL 16 · Redis 7 · MinIO (dernière release stable au démarrage, figée ensuite) · **Drizzle ORM** (décision : l'option « ou Kysely » du fichier 02 est tranchée — Drizzle) + migrations **SQL brut versionné** (drizzle-kit generate → fichiers .sql relus) · Zod 4 (schémas partagés dans `packages/shared`) · React 18 + Vite + Tailwind + shadcn/ui · Dexie 4 + dexie-react-hooks · Workbox 7 · TanStack Query 5 (console uniquement) · BullMQ 5 · pino 9 · Vitest 3 + Testcontainers + **jsdom, @testing-library/react, @vitejs/plugin-react** (AMENDEMENT du 2026-08-31, décidé par Williams : ce dépôt ne pouvait tester AUCUN composant React — `include` ne captait que `*.test.ts`, l'environnement était `node`, et aucune de ces trois briques n'était installée. Les 23 composants du design system ne pouvaient donc pas être livrés, la règle de croisement 09 §5.6 exigeant qu'un autre agent écrive leurs tests) · Playwright (dernière stable) · k6 · uuidv7 (npm) · hash-wasm (Argon2id navigateur) · date-fns · @fontsource-variable/inter (police AUTO-HÉBERGÉE — jamais de CDN de police : offline oblige, §33.1) · cmdk (Phase 2, palette console).
