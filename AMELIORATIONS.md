...
| L5d   | ~0,1 j   | 0,5 j   | ~0,4 j — repli « UTC nommé », message de repli mission, unanimité multi-mission (A22, 2026-09-08 ; fiche du 08 sous le tableau)                                                                                                                                                                                                                                                                                           |
| séc.  | ~0,05 j  | 0,5 j   | ~0,45 j — `z.config({ jitless: true })` posé pour les deux fronts, feuille `packages/shared/src/zod-sans-jit.ts` + sous-chemin d'export ; ferme la `securitypolicyviolation` `script-src` à chaque chargement (A26) ; A01 2026-09-09 ; **mesure A28 p95 < 100 ms DUE avant signature** (A21, 2026-09-09)                                                                                                                  |
| L5-PC | ~0,25 j  | 0,5 j   | ~0,25 j — trois constats de recette fermés (A22, 2026-09-09) : le mot de passe demandé est nommé « de cet appareil » sur les deux écrans qui le demandent (N3), un seul mot pour revenir en arrière et chaque retour d'écran nomme sa destination (R5), une aide clavier ouverte par « ? » et LUE dans la table des raccourcis (R7). NB-15 n'est pas compté : il ferme un critère de porte, ce n'est pas une amélioration |
